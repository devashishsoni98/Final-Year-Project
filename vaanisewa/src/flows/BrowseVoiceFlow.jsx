...
export const createBrowseFlow = (onComplete) => {
  return async (userInput, flowState) => {
    const { step, allBooks, filteredBooks, currentPage, perPage = 5, category, searchQuery } = flowState;
    ...
    switch (step) {
      case 'init': {
        ...
        if (!result.success || result.books.length === 0) {
          return {
            response: 'Sorry, I could not load books at this time. Please try again later.',
            completed: true,
            requiresInput: false,
          };
        }
      ...
      case 'browsing': {
        ...
        if (paginationCmd) {
          ...
          if (paginationCmd === 'next' && flowState.paginationInfo.hasNext) {
          ...
          } else if (paginationCmd === 'previous' && flowState.paginationInfo.hasPrevious) {
            newPage = currentPage - 1;
            ...
            if (requestedPage >= 1 && requestedPage <= flowState.paginationInfo.totalPages) {
          ...
          const summary = generatePaginationSummary(paginationInfo);
          const bookList = paginationInfo.books
            .map((book, idx) => generateBookAnnouncement(book, paginationInfo.startIndex + idx))
            .join(' ');
        ...
        return {
          response: 'Say search to find books, a category name to filter, next or previous for navigation, or a number to hear book details.',
          flowState,
          requiresInput: true,
        };
      }
